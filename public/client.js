...
function render() {
  ...
  for (let y = 0; y < n; y++)
    for (let x = 0; x < n; x++) {
      // база — трава (спрайт или клетчатый цвет), чтобы прозрачные тайлы ложились на неё
      const gspr = IMAGES['terrain:g'];
      if (gspr && gspr.ready) ctx.drawImage(gspr.img, x * CELL, y * CELL, CELL, CELL);
      else { ctx.fillStyle = (x + y) % 2 === 0 ? '#cfe0c3' : '#c6d9b9'; ctx.fillRect(x * CELL, y * CELL, CELL, CELL); }
      // поверх — не-травяной ландшафт (с выцветанием по мере истощения)
      const code = S.terrain ? S.terrain[y * n + x] : 'g';
      if (code && code !== 'g') {
        const idx = y * n + x;
        ...
        else { ctx.fillStyle = (S.terrainMeta[code] && S.terrainMeta[code].color) || '#cfe0c3'; ctx.fillRect(x * CELL, y * CELL, CELL, CELL); }
        ctx.globalAlpha = 1;
      }
      const d = districtById(dmap[y][x]);
      if (d) { ctx.globalAlpha = 0.1; ctx.fillStyle = d.color; ctx.fillRect(x * CELL, y * CELL, CELL, CELL); ctx.globalAlpha = 1; }
    }

  ctx.lineWidth = 2.5; ctx.lineCap = 'round';
  for (let y = 0; y < n; y++)
    for (let x = 0; x < n; x++) {
      const here = dmap[y][x], d = districtById(here), col = d ? d.color : '#555';
      if (x + 1 < n && dmap[y][x + 1] !== here) { ctx.strokeStyle = col; ctx.beginPath(); ctx.moveTo((x + 1) * CELL, y * CELL); ctx.lineTo((x + 1) * CELL, (y + 1) * CELL); ctx.stroke(); }
  ...
  for (const key in S.state.grid) { const [x, y] = key.split(',').map(Number); drawCell(x, y, S.state.grid[key]); }
  for (const d of S.districts) drawDistrictLabel(d);

  if (S.hover) {
    ctx.strokeStyle = S.bulldoze ? '#d98a7a' : 'rgba(42,38,34,.6)'; ctx.lineWidth = 2;
    ctx.strokeRect(S.hover.x * CELL + 1, S.hover.y * CELL + 1, CELL - 2, CELL - 2);
    // подсветка домов при наведении на школу
    const hc = S.state.grid[`${S.hover.x},${S.hover.y}`];
    if (hc && hc.type === 'school') {
      const g = schoolHighlight(S.hover.x, S.hover.y, hc);
      ctx.lineWidth = 3;
      for (const kind of ['educated', 'learning', 'blocked']) {
        ctx.strokeStyle = EDU_COLORS[kind];
        for (const h of g[kind]) ctx.strokeRect(h.x * CELL + 2, h.y * CELL + 2, CELL - 4, CELL - 4);
      }
    }
  }
  for (const c of otherCursors()) {
...
function drawCell(x, y, cell) {
  ...
  if (sp && sp.ready) {
    ...
    roundRect(px + pad, py + pad, iw, iw, 6); ctx.fillStyle = COLORS[cell.type] || '#999'; ctx.fill();
    ctx.fillStyle = '#2a2622'; ctx.font = '18px system-ui, sans-serif';
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(def.glyph, px + CELL / 2, py + CELL / 2 + 1);
    ctx.lineWidth = 2.5; ctx.strokeStyle = cell.ownerColor;
    roundRect(px + pad, py + pad, iw, iw, 6); ctx.stroke();
  }
  if (cell.type === 'house') {
    const hp = cell.happy != null ? cell.happy : 0;      // точка = счастье (красная -> зелёная)
    ctx.fillStyle = `hsl(${(hp / 100) * 120}, 70%, 44%)`;
    ctx.beginPath(); ctx.arc(px + CELL - 8, py + 8, 4.5, 0, Math.PI * 2); ctx.fill();
    ...
    if ((cell.edu || 0) >= 60) { ctx.font = '11px system-ui'; ctx.textAlign = 'left'; ctx.textBaseline = 'bottom'; ctx.fillText('🎓', px + 3, py + CELL - 3); }
  }
  const tier = cell.tier || 1;
  if (tier > 1 && cell.type !== 'center') { // точки уровня внизу слева
    for (let i = 0; i < tier; i++) {
      ctx.fillStyle = '#e6b34a'; ctx.beginPath(); ctx.arc(px + 6 + i * 6, py + CELL - 6, 2.4, 0, Math.PI * 2); ctx.fill();
    }
  }
  if (inactive) {
    ctx.globalAlpha = 1;
